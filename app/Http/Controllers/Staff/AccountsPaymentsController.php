...
class AccountsPaymentsController extends Controller
{
     ...
     * Allows accounts to view draft applications (read-only, view only)
     */
    ...
    /**
     * View a specific draft application (read-only)
     * Note: Accounts can only view, not review (no action buttons)
     */
    public function viewDraft(Application $draft)
    {
        // Ensure it's actually a draft
        if ($draft->status !== Application::DRAFT) {
            return redirect()->route('staff.accounts.drafts')
