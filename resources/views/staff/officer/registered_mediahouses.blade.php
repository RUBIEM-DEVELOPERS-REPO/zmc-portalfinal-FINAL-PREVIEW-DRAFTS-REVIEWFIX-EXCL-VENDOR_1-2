...
<div class="zmc-dashboard-wrapper" style="font-family:'Roboto', sans-serif; color:#334155; padding: 20px;">
  ...
  <ul class="nav nav-tabs mb-4 px-1 border-bottom border-2">
    <li class="nav-item">
      <a class="nav-link fw-semibold text-muted border-0" href="{{ route('staff.officer.records.accredited-journalists') }}">
        <i class="ri-user-star-line me-1"></i> Accredited Media Practitioners
      </a>
    </li>
    <li class="nav-item">
      <a class="nav-link active fw-bold text-dark border-0 border-bottom border-dark border-3" href="{{ route('staff.officer.records.registered-mediahouses') }}" style="background: transparent;">
  ...
  <div id="databaseMode">
    ...
    <div class="zmc-card mb-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="d-flex gap-2">
          <a href="{{ route('staff.officer.records.registered-mediahouses.export') }}?{{ http_build_query(request()->query()) }}" class="btn btn-sm btn-outline-success">
          ...
          <div class="row g-3">
            <div class="col-12 col-md-3">
              ...
              <select name="ownership_type" class="form-select">
                ...
                <option value="local" {{ request('ownership_type') == 'local' ? 'selected' : '' }}>Local</option>
                <option value="foreign" {{ request('ownership_type') == 'foreign' ? 'selected' : '' }}>Foreign</option>
                <option value="joint_venture" {{ request('ownership_type') == 'joint_venture' ? 'selected' : '' }}>Joint Venture</option>
              </select>
            </div>
            ...
            <div class="col-12 col-md-3">
              ...
              <select name="media_type" class="form-select">
                ...
                <optgroup label="Mass Media Service Categories">
                  <option value="MC" {{ request('media_type') == 'MC' ? 'selected' : '' }}>MC — Community Media</option>
                  <option value="MA" {{ request('media_type') == 'MA' ? 'selected' : '' }}>MA — Advertising agency as media service</option>
                  <option value="MF" {{ request('media_type') == 'MF' ? 'selected' : '' }}>MF — Local office for foreign media service</option>
                  <option value="MN" {{ request('media_type') == 'MN' ? 'selected' : '' }}>MN — National media service publishing newspaper</option>
                  <option value="DG" {{ request('media_type') == 'DG' ? 'selected' : '' }}>DG — Internet base media service</option>
                  <option value="MP" {{ request('media_type') == 'MP' ? 'selected' : '' }}>MP — Production house as media service</option>
                  <option value="MS" {{ request('media_type') == 'MS' ? 'selected' : '' }}>MS — National media service publishing magazine</option>
                  <option value="MT" {{ request('media_type') == 'MT' ? 'selected' : '' }}>MT — Broadcasting media service free to air</option>
                  <option value="MB" {{ request('media_type') == 'MB' ? 'selected' : '' }}>MB — Satellite broadcast</option>
                  <option value="MV" {{ request('media_type') == 'MV' ? 'selected' : '' }}>MV — Video on demand</option>
                </optgroup>
              </select>
            </div>
            ...
            <div class="col-12 col-md-3">
              ...
              <select name="business_type" class="form-select">
                <option value="">All Types</option>
                <option value="private" {{ request('business_type') == 'private' ? 'selected' : '' }}>Private</option>
                <option value="public" {{ request('business_type') == 'public' ? 'selected' : '' }}>Public</option>
                <option value="government" {{ request('business_type') == 'government' ? 'selected' : '' }}>Government</option>
                <option value="ngo" {{ request('business_type') == 'ngo' ? 'selected' : '' }}>NGO</option>
              </select>
            </div>
            ...
            <div class="col-12 col-md-2">
              ...
              <input type="text" name="reg_number" class="form-control" 
                     value="{{ request('reg_number') }}" placeholder="Business registration">
            </div>
            
            <div class="col-12 col-md-2">
              <label class="form-label small fw-bold">Tax Number</label>
              <input type="text" name="tax_number" class="form-control" 
                     value="{{ request('tax_number') }}" placeholder="Tax/BVR number">
            </div>
          </div>
        </div>
      </form>
        ...
        <table class="table table-hover align-middle mb-0" style="font-size: 12px;">
          ...
          <tbody>
            @forelse($mediaHouses as $mediahouse)
              ...
              @endphp
              <tr>
                <td class="fw-bold">{{ $mediahouse->registration_no ?? '—' }}</td>
                <td>
                ...
                <td>{{ $formData['trading_name'] ?? $formData['trading_as'] ?? '—' }}</td>
                <td>
                  <span class="badge bg-info">{{ $formData['business_type'] ?? '—' }}</span>
                </td>
                <td>{{ $formData['business_registration'] ?? $formData['company_registration'] ?? '—' }}</td>
                <td>{{ $formData['tax_number'] ?? $formData['bvr_number'] ?? '—' }}</td>
                <td>
                  <span class="badge bg-{{ $formData['ownership_type'] === 'foreign' ? 'warning' : 'primary' }}">
                    {{ ucfirst($formData['ownership_type'] ?? '—') }}
                  </span>
                </td>
                <td>{{ $formData['shareholding_percentage'] ?? $formData['local_ownership'] ?? '—' }}%</td>
                <td>{{ $formData['contact_person_title'] ?? '—' }}</td>
                <td>
                  <div class="fw-semibold">{{ $contact?->name ?? $formData['contact_person_name'] ?? '—' }}</div>
                  <div class="small text-muted">{{ $formData['contact_person_surname'] ?? '' }}</div>
                </td>
                <td>{{ $formData['contact_person_id_number'] ?? $contact?->id_number ?? '—' }}</td>
                <td>{{ $formData['physical_address'] ?? $formData['address'] ?? '—' }}</td>
                <td>{{ $formData['postal_address'] ?? $formData['mailing_address'] ?? '—' }}</td>
                <td>
                  <div class="small">{{ $contact?->phone ?? $formData['phone_number'] ?? '—' }}</div>
                  <div class="small">{{ $contact?->phone ?? $formData['mobile_number'] ?? '—' }}</div>
                </td>
                <td>
                  <div class="small text-truncate" style="max-width: 150px;" title="{{ $contact?->email ?? $formData['email_address'] ?? '' }}">
                    {{ $contact?->email ?? $formData['email_address'] ?? '—' }}
                  </div>
                </td>
                <td>
                  <span class="badge bg-success">{{ $formData['media_type'] ?? '—' }}</span>
                </td>
                <td>
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      <li><a class="dropdown-item" href="#" onclick="viewFullRecord({{ $mediahouse->id }})">
                        <i class="ri-eye-line me-1"></i>View Full Record
                      </a></li>
                      <li><a class="dropdown-item" href="#" onclick="editRecord({{ $mediahouse->id }})">
                        <i class="ri-edit-line me-1"></i>Edit Record
                      </a></li>
                      <li><a class="dropdown-item" href="#" onclick="downloadDocuments({{ $mediahouse->id }})">
    ...
    <div class="row g-4 mb-4">
      <div class="col-12 col-lg-6">
        <div class="zmc-card h-100">
          <h6 class="fw-bold mb-3"><i class="ri-bar-chart-grouped-line me-2"></i>Registrations by Type ({{ $currentYear }})</h6>
      ...
      <div class="col-12 col-lg-6">
        <div class="zmc-card h-100">
          <h6 class="fw-bold mb-3"><i class="ri-line-chart-line me-2"></i>Registrations Over Time ({{ $currentYear }})</h6>
          <div style="height: 300px; position: relative;">
...
function openEditRecordModal(type, id) {
    ...
    fetch(`{{ url('staff/accreditation-officer/records') }}/${id}/edit-data?type=${type}`)
        .then(r => r.json())
        .then(res => {
