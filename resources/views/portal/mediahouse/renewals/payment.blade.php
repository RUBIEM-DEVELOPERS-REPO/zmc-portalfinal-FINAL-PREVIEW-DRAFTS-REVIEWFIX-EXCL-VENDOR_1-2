...
async function submitPaynow() {
  ...
  try {
    const response = await fetch('{{ route("mediahouse.renewals.payment.paynow", $renewal) }}', {
      method: 'POST',
      headers: {
    ...
    if (data.ok) {
      alert(data.message);
      window.location.href = '{{ route("mediahouse.renewals.show", $renewal) }}';
    } else {
...
async function submitProof() {
  ...
  try {
    const response = await fetch('{{ route("mediahouse.renewals.payment.proof", $renewal) }}', {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': '{{ csrf_token() }}'
      },
